...
`MapView` owns pan, zoom, and selected-territory camera movement. The `viewBox` is bounded to the map rectangle, so no off-map area is visible. On initial load, the camera shows the largest current-screen-shaped rectangle that fits inside the map. The full map is visible only when the screen aspect matches the map aspect. When a territory is selected, `MapView` fits that territory's generated `focusBounds` rectangle to the current screen shape, then clamps the result inside the map. Nearly identical focus moves happen instantly; visible moves use a short distance-based ease-in-out animation. The focus rectangle is generated from the canonical territory fill loops with 500 map units of padding, except on sides where the territory touches the outer map edge.
