...
Complete game-state transitions belong in `src/game/gameState.ts`. Pausing and resuming are handled by `pauseGame(...)` and `resumePausedGame(...)`, including draft/allocation timer preservation, sync connected-player checks, and paused map return behavior. `App.tsx` may clear local UI selections before pausing, but it must not duplicate phase-by-phase pause/resume state transitions inline.
