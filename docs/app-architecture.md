...
Persistent section changes do not resize the map and do not mutate camera state. `MapView` receives measured visible insets from the app shell and uses them only when an explicit camera target is requested. Return-to-map fits `homeViewport` inside the visible aperture. Automatic selected-territory focus fits generated focus bounds inside that same aperture. Manual pan/zoom and the current `viewBox` remain full-screen SVG camera state and are not synced or persisted.
