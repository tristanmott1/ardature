...
Map press handling has one contract. `src/game/gameView.ts` projects the active `MapPressMode`, the local selection update caused by a territory press, selection cleanup, and selection patch merging. `App.tsx` wires the map callback and applies the returned local selection patch; it should not duplicate a second mode switch for allocation, draft, inspect, reinforcement, spy selection, or selection cleanup.
