...
Host transfer is a sync pause authority operation. It is available only to the current connected host while paused, and only to a currently connected non-host player. Resume is disabled during transfer. The old host remains source of truth until the selected player acknowledges the latest snapshot and becomes the new broadcaster/recovery authority. Because sync is host-centered, existing channels cannot be magically reassigned; the old host's still-live channels are used only as a temporary signaling bridge so connected peers can establish channels to the new host. No gameplay may resume until the new host has the authoritative snapshot and every remaining player is either connected to the new host or explicitly disconnected for the new host's recovery flow. If the host is eliminated while at least two players remain, confirming that elimination forces pause and requires host transfer before resume; after transfer succeeds, the old host is removed/disconnected and returned home.
