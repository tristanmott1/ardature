...
When layout sections change the rendered map size, `MapView` preserves camera continuity instead of letting the same `viewBox` stretch into a new shape. A resize observer keeps the current map scale as much as bounds allow and anchors the map coordinate under the old/new overlap center to the same screen position. This is presentation-only camera state; it is not synced or persisted. Focus and return-to-map animations override resize preservation.
