...
Allocation-style troop rows expose bulk `+` and `-` buttons through `TroopPlacementRows`. The parent surface owns the legality and maximum-move calculation, so allocation, reinforcement, attack, and fortify reuse the row UI without pushing game rules into the component. When a bulk move must leave one troop behind, the shared priority is heavy, then cavalry, then elite, then leader.
