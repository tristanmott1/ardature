...
Paths of the Dead is gated by `pathsOfTheDeadState`, also stored in `GameState`. It is `null` before the first regular turn. When regular turns begin, the first state is uniformly sampled from `1-6`. The generated graph contains `Edoras -> Lamedon` and never contains `Lamedon -> Edoras`. States `1-3` remove `Edoras -> Lamedon` from the active graph. States `4-6` leave that one directed edge active.
...
The separate challenge test page opened from the home page is a development sandbox only. It is not used by battle resolution yet and should not change gameplay state. It currently ports OpenPigeon archery as a self-contained Three.js scene with copied local assets. It uses `6.2` pointer sensitivity, `1000` maximum cursor speed, sine-based breathing drift of `12px` horizontal over `1500ms` and `7px` vertical over `1000ms`, randomized drift phases per aim, camera draw zoom to FOV `41.5`, `500ms` early-release cancel threshold, `2500ms` visible auto-fire fill, target ring math, and stuck-arrow behavior. The progress ring appears when the draw completes and auto-fires at `3000ms` total hold time. Arrow travel takes `500ms` at the standard `1x` distance and scales by actual shot distance so arrows keep the same world speed at every target distance. The visible cursor is drawn from the same animation-loop state used for shot projection, the camera matrix is refreshed before projection, the wind indicator points in the same screen-space direction that wind pushes the hit, and the imported arrow is tip-anchored so the visual tip lands at the computed hit point. The sandbox target is generated as no-number colored rings, placed at the far OpenPigeon distance at `1x`, and paired with harder `2.5..5.0` ring-unit wind. A bottom-left distance button opens a slider from `0.25x` to `2x` in `0.05x` steps. Distance changes move the target rig, clear stuck arrows, and keep attempts, mean score, and shot-distribution buckets intact. Wind and player error stay constant relative to the screen as distance changes, so farther targets make the same shot error cover more target-ring units. Each shot maps radius to continuous score as `score = 10 * Beta(2,2).inverseCDF(1 - GammaCDF(radius))`, using the tuned average-player radius model `Gamma(shape = 3.25, scale = 0.76)`. The target rings are the score-boundary radii `0.3568`, `0.7784`, `1.1641`, `1.5597`, `1.9881`, `2.4733`, `3.0512`, `3.7895`, `4.8579`, and `7.0211`. `Mean Score` is the running average of continuous shot scores. The bottom score bar has a plot button that opens a simple empirical distribution chart. Shots are bucketed by score: `10` is score at least `9.5`, `1` is score at least `0.5`, and `<1` is below `0.5`. Bars use percentages on the y-axis, while labels above bars show raw counts. Restart clears the chart counts with the rest of the sandbox metrics, but keeps the selected distance multiplier.
...
Score-distribution review should be done with `scripts/plot_challenge_distance.py`. Run it through Poetry with `poetry run python scripts/plot_challenge_distance.py`. It writes plots and CSV tables to `verification-output/challenge-calibration/`. The plot uses only the fitted gamma radius model, the current target-ring score mapping, and screen-relative distance scaling: changing target distance multiplies the effective radius in target-ring units because player error and wind keep the same effect relative to the screen, not relative to the changing target size.

Use `poetry run python scripts/plot_challenge_distance.py` to plot score distributions by target-distance multiplier. This plotting tool uses only the fitted average-player gamma radius model, the current target-ring score mapping, and the rule that wind/player error stays constant relative to the screen. Distance therefore multiplies the effective radius before scoring: closer targets shift the score distribution upward, farther targets shift it downward. The script writes CSV plus PNG/SVG plots to `verification-output/challenge-calibration/`.
...
Both sides' current battle troop breakdowns are visible during the battle. Everyone who can see the battle modal sees the same battle contents, captured spies present with the battle forces, and which troop types die. Captured spies are not battle units, casualties, or dice candidates. The modal shows the latest roll only, not a full roll history. The latest normal roll always shows exactly the dice that were rolled, sorted highest to lowest from left to right for both attacker and defender. Dice never show troop-icon badges; troop identity is game-state data for casualty resolution, not displayed dice UI. Casualties change troop rows immediately and change the next roll's dice count, but they do not remove dice from the just-finished roll.

When the latest roll is a Balrog roll, no extra message is shown. The non-looping Balrog GIF at `public/balrog/balrog.gif` fills the full modal background at 50% opacity for `1400ms`; it uses a fresh per-roll image URL so cached non-looping playback always starts from the first frame, uses cover-style sizing, may bleed beyond the modal bounds, and is clipped by the modal. The selected dice immediately appear as black blank dice with no pips or troop badges. The dice and retreat controls remain disabled until the GIF completes.
