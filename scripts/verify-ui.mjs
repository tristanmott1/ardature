...
async function runSourceChecks() {
  ...
  assert(gameViewSource.includes("type TroopSectionMode") && gameViewSource.includes('type: "allocation"') && gameViewSource.includes('type: "allocationWaiting"') && gameViewSource.includes('type: "info"') && !gameViewSource.includes("type StatusSectionMode") && !appSource.includes("function renderStatusSection()") && !appSource.includes("showAllocationTroopSection"), "Troop, waiting, and info content all use the single troop-section projection.");
