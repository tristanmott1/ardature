...
async function runSourceChecks() {
  ...
  assert(appSource.includes("type MapPressMode") && appSource.includes("function mapPressModeForGame") && appSource.includes("switch (mapPressMode)") && appSource.includes("onTerritoryPress={!freezeMapGestures && mapPressMode ? pressTerritory : undefined}"), "Map territory presses use one explicit mode contract.");
