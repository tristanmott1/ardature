...
async function runSourceChecks() {
  ...
  assert(syncMessagesSource.includes('type: "dismissNotification"') && syncMessagesSource.includes("notificationId: string") && appSource.includes('sendTurnCommand({ type: "dismissNotification", notificationId: currentNotification.id })'), "Sync joiners dismiss queued notifications through the host by notification id.");
  ...
  assert(appSource.includes("type MapPressMode") && appSource.includes("function mapPressModeForGame") && appSource.includes("switch (mapPressMode)") && appSource.includes("onTerritoryPress={!layout.freezeMapGestures && mapPressMode ? pressTerritory : undefined}"), "Map territory presses use one explicit mode contract.");
  ...
  assert(appSource.includes('const isGameStage = game.phase !== "home" && game.phase !== "setup"') && appSource.includes("showPlayerBar: isGameStage && Boolean(playerBarPlayer)") && appSource.includes("showGameStageLayout: isGameStage"), "Game-stage layout and player bar are not gated by overlay-specific draft state.");
  assert(appSource.includes("type GameStageLayout") && appSource.includes("function gameStageLayoutForState") && appSource.includes("const layout = gameStageLayoutForState") && appSource.includes("canUseMapCameraControls: isGameStage && !hasActiveOverlay && !showAllocationWaitingSection"), "Overlay policy derives section hiding, map freezing, and camera controls from one layout projection.");
  ...
  assert(appSource.includes("function playerBarPlayerForGame") && appSource.includes("function playerBarDraftProgress") && appSource.includes("const playerBarPlayer = playerBarPlayerForGame") && !appSource.includes("const playerBarIsDraft ="), "Persistent player-bar identity and progress use named helpers.");
